...
export function renderArtifactPanel(title: string, rows: ArtifactRow[], out: NodeJS.WritableStream = process.stdout): string {
...
export function renderExecutionSummary(report: ExecutionReport, baseDir: string, out: NodeJS.WritableStream = process.stdout): string {
  const colorEnabled = streamSupportsColor(out);
  ...
  const stats: ArtifactRow[] = [
    ...
    ...(summary.confirmedFindings + summary.inconclusiveFindings > 0
      ? [{ label: "findings", value: `${summary.confirmedFindings} confirmed, ${summary.inconclusiveFindings} inconclusive` }]
      : []),
    ...(summary.safetyBlockedRequests > 0 ? [{ label: "safety blocked", value: `${summary.safetyBlockedRequests} request(s)` }] : []),

...
const SENSITIVE_KEY = /(?:^|[-_])(api[-_]?key|authorization|cookie|set[-_]?cookie|password|passwd|secret|token|access[-_]?token|refresh[-_]?token|id[-_]?token|storage(?:state)?|credential|client[-_]?secret|private[-_]?key|session(?:[-_]?id)?)(?:$|[-_])/i;
const SENSITIVE_QUERY = /(api[-_]?key|authorization|cookie|password|passwd|secret|token|access_token|refresh_token|session|credential)/i;
...
const KEY_VALUE_SECRET = /\b(password|passwd|api[_-]?key|authorization|token|secret|client[_-]?secret)\s*[:=]\s*([^,\s}]+)/i;
const KEY_VALUE_SECRET_REPLACE = /\b(password|passwd|api[_-]?key|authorization|token|secret|client[_-]?secret)\s*[:=]\s*([^,\s}]+)/gi;
const ABSOLUTE_URL = /https?:\/\/[^\s"'<>]+/gi;
const RELATIVE_URL_WITH_QUERY = /(^|[\s(])((?:\/|\?)[^\s"'<>]*\?[^\s"'<>]+)/g;
const ANSI_ESCAPE = /\u001b\[[0-?]*[ -/]*[@-~]/g;
const ENCODED_ANSI_ESCAPE = /%1b\[[0-?]*[ -/]*[@-~]/gi;
const SENSITIVE_LOCATOR = /password|passwd|secret|token|authorization|api[-_ ]?key|card(?:[-_ ]?number)?|cvv|cvc|ssn|social[-_ ]?security/i;
...
function isSensitiveKey(key: string): boolean {
  return SENSITIVE_KEY.test(key) || /(?:token|secret|password|credential|authorization|cookie|apikey|privatekey|storage(state)?|sessionid)$/.test(normalizeKey(key));
}
...
function redactString(value: string, secrets: readonly string[], options: RedactOptions): string {
  ...
  result = result.replace(SENSITIVE_TEXT_REPLACE, (_match, prefix: string | undefined) => prefix ? `${prefix}${REDACTED_VALUE}` : REDACTED_VALUE);
  ...
  result = result.replace(RELATIVE_URL_WITH_QUERY, (_match, prefix: string, url: string) => `${prefix}${redactUrlValue(url, options)}`);
...
export class Redactor {
  ...
  constructor(secrets: readonly (string | undefined)[] = []) {
    this.secrets = [...new Set(secrets.filter((secret): secret is string => typeof secret === "string" && secret.length >= 3))]
  ...
  hasSensitiveData(value: unknown): boolean {
    ...
    if (typeof value === "string") {
      return this.secrets.some((secret) => value.includes(secret)) || SENSITIVE_TEXT.test(value) || KEY_VALUE_SECRET.test(value);
    }

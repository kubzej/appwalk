...
export function providerHttpError(
  ...
  return new ProviderRequestError(
    ...
    { provider, model, requestIndex, attempt: 0, status, retryable: RETRYABLE_STATUSES.has(status), headers },
  );
}
...
function isNetworkError(error: unknown): boolean {
  ...
  return (typeof code === "string" && /ECONNRESET|ECONNREFUSED|ETIMEDOUT|EAI_AGAIN|ENETUNREACH/i.test(code))
  ...
  for (let attempt = 0; attempt <= HOSTED_PROVIDER_MAX_RETRIES; attempt++) {
    ...
    if (remainingMs <= 0) {
      throw new ProviderRequestError(
        ...
        { provider: options.provider, model: options.model, requestIndex: options.requestIndex, attempt, retryable: false },
      );
    }
    ...
    try {
      ...
      attemptSignal = createAttemptSignal(options.signal, Math.min(HOSTED_PROVIDER_REQUEST_TIMEOUT_MS, requestRemainingMs));
      ...
      if (externallyAborted) {
        throw new ProviderRequestError(
          ...
          { provider: options.provider, model: options.model, requestIndex: options.requestIndex, attempt, status: providerStatus(error), retryable: false },
          { cause: error },
        );
      }
      ...
      const failureDetail = timedOut
        ? `timed out after ${HOSTED_PROVIDER_REQUEST_TIMEOUT_MS}ms`
        : errorMessage(error);
      if (!retryable || attempt >= HOSTED_PROVIDER_MAX_RETRIES) {
        ...
        throw new ProviderRequestError(
          `${options.provider} request failed after ${attempt + 1} attempt${attempt === 0 ? "" : "s"}: ${failureDetail}`,
          { provider: options.provider, model: options.model, requestIndex: options.requestIndex, attempt, status: providerStatus(error), retryable },
          { cause: error },
        );
      }

      const requestedWaitMs = options.retryDelayMs?.(error, attempt) ?? Math.min(1_000 * 2 ** attempt, MAX_RATE_LIMIT_WAIT_MS);
      const waitMs = Math.min(Math.max(0, requestedWaitMs), MAX_RATE_LIMIT_WAIT_MS, deadline - Date.now());
      if (waitMs <= 0) {
        throw new ProviderRequestError(
      ...
      options.logger?.warn(`${options.provider} request failed transiently; retrying in ${Math.ceil(waitMs / 1000)}s (attempt ${attempt + 2}/${HOSTED_PROVIDER_MAX_RETRIES + 1}).`);
      options.logger?.debug("provider.retrying", "Retrying a transient provider request", {
        provider: options.provider, model: options.model, requestIndex: options.requestIndex, attempt, waitMs, status: providerStatus(error), error: errorMessage(error),
      });
      try {

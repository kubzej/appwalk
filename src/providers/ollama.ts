import type { LlmProvider, ProviderTurn, ToolDefinition, ToolResult } from './provider.js';
...
export class OllamaProvider implements LlmProvider {
  ...
  private async send(): Promise<ProviderTurn> {
    ...
    this.logger.debug('provider.request_started', 'Ollama request started', {
      provider: 'ollama',
      model: this.model,
      requestIndex,
    });
    ...
    if (toolCalls.length > 1) {
      this.logger.debug(
        ...
        'Ollama returned multiple tool calls; processing only the first',
        {
          provider: 'ollama',
          model: this.model,
          requestIndex,

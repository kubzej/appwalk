...
export class GrokProvider implements LlmProvider {
  ...
  private async send(
    ...
    this.logger.debug('provider.request_started', 'Grok request started', {
      provider: 'grok',
      model: this.model,
      requestIndex,
    });
    ...
    const result = await withHostedProviderRequest<{ data: GrokResponse; headers: Headers }>(
      async (attemptSignal) => {
    ...
    if (functionCalls.length > 1) {
      this.logger.debug(
        ...
        'Grok returned multiple tool calls; processing only the first',
        {
          provider: 'grok',
          model: this.model,
          requestIndex,

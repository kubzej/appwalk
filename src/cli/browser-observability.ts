...
export function attachPopupDetection(page: Page, logger: Logger, tabRegistryHandle?: TabRegistryHandle): void {
  ...
  page.on('popup', (popup) => {
    ...
    if (tabRegistryHandle) {
      ...
      logger.debug('browser.popup_registered', 'Popup registered as an addressable tab', {
        tabId: newId,
        url: popup.url(),
      });
    }
...
export function attachWebSocketCapture(page: Page, recorder: EvidenceRecorder): void {
  ...
  page.on('websocket', (ws) => {
    ...
    ws.on('framesent', ({ payload }) =>
      recorder.recordWebSocketFrame({ url, direction: 'sent', payload: webSocketPayloadToText(payload) }),
    );
    ws.on('framereceived', ({ payload }) =>
      recorder.recordWebSocketFrame({ url, direction: 'received', payload: webSocketPayloadToText(payload) }),
    );
  });
}
...
export async function createTraceSession(
  ...
  const segmentPath = () => (segment === 1 ? basePath : basePath.replace(/\.zip$/i, `-part-${segment}.zip`));

...
test('a context-level guard blocks a destructive request from a second page in the same context, with no reinstall', async () => {
  ...
  try {
    ...
    await firstPage.evaluate(async () => {
      try {
        await fetch('https://app.test/api/cart', { method: 'POST' });
      } catch {
        /* aborted */
      }
    });
    ...
    await secondPage.evaluate(async () => {
      try {
        await fetch('https://app.test/api/cart', { method: 'DELETE' });
      } catch {
        /* aborted */
      }
    });
...
test('a context-level guard allows non-destructive requests, and installing it twice is a no-op', async () => {
  ...
  try {
    ...
    await page.evaluate(async () => {
      try {
        await fetch('https://app.test/api/cart', { method: 'GET' });
      } catch {
        /* real network attempt, may fail for unrelated DNS reasons */
      }
    });
...
test('allowDestructive disables default method blocking but preserves explicit URL blocks', () => {
  ...
  assert.equal(evaluateSafetyRequest('POST', 'https://app.test/api/admin/users', options).blocked, true);
  assert.equal(evaluateSafetyRequest('GET', 'https://app.test/api/admin/health', options).blocked, false);
  assert.equal(evaluateSafetyRequest('POST', 'https://app.test/api/orders', options).blocked, false);
});

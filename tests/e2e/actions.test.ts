...
test('dialog handlers are discarded when the following action does not consume them', async () => {
  ...
  try {
    ...
    assert.match(
      result.snapshot,
      /dialog handler was armed but no dialog appeared during this action; it was discarded/,
    );
...
test('clearUnusedDialogHandler removes the exact armed listener', () => {
  ...
  const page = {
    once: (_event: string, listener: unknown) => {
...
test('apiRequest rejects a non-GET/HEAD method at runtime, not just via the tool schema', async () => {
  ...
  try {
    ...
    await assert.rejects(
      executeToolCall(page, {
        id: '1',
        name: 'apiRequest',
        input: { method: 'DELETE', url: 'https://this-domain-does-not-exist.invalid/api/thing' },
      }),
    ...
    await page.route('https://app.test/**', async (route) => {
      if (route.request().url().endsWith('/page')) {
        await route.fulfill({
          ...
          body: `<a id="download" href="/broken.bin" download>Download</a>`,
        });
        return;
      }
...
test('setOffline drops every request in the context, not just one armed request, and restores it again', async () => {
  ...
  try {
    ...
    const firstFailed = await page.evaluate(
      (p) =>
        fetch(`http://127.0.0.1:${p}/api/one`)
          .then(() => false)
          .catch(() => true),
      port,
    );
    const secondFailed = await page.evaluate(
      (p) =>
        fetch(`http://127.0.0.1:${p}/api/two`)
          .then(() => false)
          .catch(() => true),
      port,
    );
...
test('openTab and switchTab track multiple pages by id and share live storage like real browser tabs', async () => {
  ...
  try {
    ...
    const switchResult = await executeToolCall(
      newTabPage,
      { id: '2', name: 'switchTab', input: { tabId: 'tab-0' } },
      tabs,
    );

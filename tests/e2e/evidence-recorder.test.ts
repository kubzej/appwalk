...
test('captures network and console activity from a second page in the same context with no reattach', async () => {
  ...
  try {
    ...
    assert.ok(
      urls.some((url) => url === 'https://app.test/'),
      "first page's navigation was captured",
    );
    assert.ok(
      urls.some((url) => url === 'https://app.test/second'),
      "second page's navigation was captured with no reattach",
    );
    assert.ok(
      recorder.runtimeErrors.some(
        (error) => error.kind === 'console_error' && error.message.includes('second-page-error'),
      ),

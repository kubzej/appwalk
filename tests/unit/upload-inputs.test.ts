...
import { DEFAULT_UPLOAD_INPUT_ROOT, UploadInputPolicy } from '../../src/security/upload-inputs.js';

test("default upload input policy accepts only the project's approved agent input", () => {
  const [resolved] = new UploadInputPolicy().resolvePaths(['agent-inputs/uploads/uma/valid.png']);
...
test('upload input policy rejects absolute, traversal, and outside-root paths', () => {
  ...
  assert.throws(() => policy.resolvePaths(['package.json']), /must be inside agent-inputs\/uploads/);
});

...
test('normal output stays concise and omits diagnostic details', () => {
  const output = outputFor('normal', (logger) =>
    logger.info('Exploration started', { action: 1, url: 'https://example.test' }),
  );
  assert.equal(output, 'Exploration started\n');
});

test('verbose output includes useful details while redacting secrets', () => {
  const output = outputFor('verbose', (logger) =>
...
test('debug output includes a stable event name', () => {
  const output = outputFor('debug', (logger) =>
    logger.debug('agent.turn_started', 'Agent context started', { actionCount: 0 }),
  );
...
test('action failures are warnings in normal output but muted in verbose output', () => {
  let normal = '';
  let verbose = '';
  const normalStream = new Writable({
    write(chunk, _encoding, done) {
      normal += chunk.toString();
      done();
    },
  });
...
test('verbose output keeps the action tree clean while debug retains context', () => {
  ...
  new Logger('debug', debugStream, { runId: 'run-1', persona: 'noah' }).debug('agent.step', 'Action completed', {
...
test('task() reports failure and rethrows', async () => {
  ...
  await assert.rejects(
    () =>
      logger.task('Launching browser', async () => {

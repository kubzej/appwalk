...
test('keeps repeated responses and assigns occurrences', () => {
  ...
  assert.deepEqual(
    fixtures.map((fixture) => fixture.occurrence),
    [1, 2],
  );
...
test('explains when the planner returns no proposals', () => {
  const fixtures = extractResponseFixtures(evidence(), applicationUrl);
  const result = parseResponseVariantsDetailed(
    JSON.stringify({
      variants: [],
  ...
  assert.equal(
    result.reason,
    'Planner returned no variant proposals: No observable alternate order state was available.',
  );
});
